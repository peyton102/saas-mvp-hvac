...
def review_link(tenant_id: str, db: "Optional[object]" = None) -> Optional[str]:
    ...
    if db is not None:
        try:
            from sqlmodel import select  # lazy import
            from app.models import Tenant  # lazy import
...
def brand(tenant_id: str, db: Optional[Session] = None) -> Dict[str, Any]:
    ...
    Priority:
      1) TENANT_BRANDS[tenant_id][KEY]
      2) Global config / settings
      3) DB Tenant row (if db session provided)
    """
    overrides = _tenant_overrides().get(tenant_id, {})

    # 1) base values from overrides / global config
    FROM_NAME = (
    ...
    # 2) DB overrides if a Session is provided
    if db is not None:
        try:
            t = db.exec(select(Tenant).where(Tenant.slug == tenant_id)).first()
        except Exception:
...
# ---------------- generic branding/contact settings ----------------
...
def update_tenant_settings(
    ...
    """
    Portal-only: use the logged-in user to find the tenant.
    This ignores X-API-Key / default and always uses the JWT.
    """
    tenant_slug = current_user.get("tenant_slug") or current_user.get("tenant")
    if not tenant_slug:
        raise HTTPException(status_code=500, detail="Missing tenant in auth payload")
...
def get_tenant_settings(
    ...
    """
    Portal-only: use the logged-in user to select the tenant.
    """
    tenant_slug = current_user.get("tenant_slug") or current_user.get("tenant")
    if not tenant_slug:
        raise HTTPException(status_code=500, detail="Missing tenant in auth payload")
...
def debug_tenant_brand(
    ...
    """
    Debug endpoint can still use get_tenant_id (API key / default) as before.
    """
