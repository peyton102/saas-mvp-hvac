...
def availability(start: str | None = None, end: str | None = None):
    try:
        from ..services.google_calendar import ensure_service, load_creds, freebusy, generate_slots
        import os
        from datetime import datetime, timedelta, timezone
        from dateutil import parser as dateparser
        ...
        tz_str = os.getenv("TZ", "America/New_York")
        cal_ids = [c.strip() for c in os.getenv("GOOGLE_CALENDAR_IDS", "primary").split(",") if c.strip()]
        bh = os.getenv("BUSINESS_HOURS", "09:00-17:00")
        slot_minutes = int(os.getenv("SLOT_MINUTES", "60"))
        buffer_minutes = int(os.getenv("SLOT_BUFFER_MINUTES", "0"))
        ...
        slots = generate_slots(
            start=start_dt, end=end_dt, tz_str=tz_str, busy=busy,
            business_hours=bh, slot_minutes=slot_minutes, buffer_minutes=buffer_minutes,
        )
        return {"ok": True, "authorized": True, "count": len(slots), "slots": slots}
    except Exception as e:
        from fastapi import HTTPException
        raise HTTPException(status_code=500, detail=f"Availability failed: {e}")
